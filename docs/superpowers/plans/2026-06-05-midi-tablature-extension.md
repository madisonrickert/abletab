...
function buildSettings(): alphaTab.Settings {
  ...
  // Bundle Bravura inline so rendering needs no network/file access.
...
Expected: PASS. The AlphaTab API points used here were verified against the installed 1.6 typings: `FontFileFormat.Woff2`, the `AlphaTexImporter` (note: the alphaTex string importer is `AlphaTexImporter`, not `AlphaTabImporter`), `Staff.showStandardNotation`/`showTablature`, and the `ScoreRenderer` events whose arg is `RenderFinishedEventArgs` (`id`, `renderResult`, `totalWidth`, `totalHeight`). If any errors, open `node_modules/@coderline/alphatab/dist/alphaTab.d.ts` and correct the name to match.
