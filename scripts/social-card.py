...
# dependencies = []
# ///
"""Generate the GitHub social-preview card for the AbleTab extension.
...
the GitHub social-preview spec. Shared generator across the extension family
(AbleVSEP, Sheet Music, AbleTab).
...
Build a clean hero by cropping the screenshot to its opaque window:
  ...
  magick docs/screenshot.png -crop "$bbox" +repage -background white -alpha remove -alpha off /tmp/abletab-hero.png

Usage:
  uv run scripts/social-card.py --title "AbleTab" \
      --tagline "See any MIDI clip as tablature. Pick a tuning and export PDF & ASCII tab." \
      --hero /tmp/abletab-hero.png --out docs/social-preview.png
